    ...
    return (
        <div className='flex w-full overflow-hidden dark:hover:bg-slate-800/30 hover:bg-slate-100  rounded-md min-h-[5rem] h-20'>
            <div className='h-full w-12  grid place-items-center'>
                <FontAwesomeIcon
                    className='h-5 text-slate-400 dark:text-slate-700'
                    icon={faQuoteRight}
                />
            </div>
            <div className='flex py-1 pr-2 justify-center  flex-col w-full h-full'>
                <h1 className='dark:text-slate-400 text-slate-800 font-medium'>{author}</h1>
                <p className='dark:text-slate-600 text-slate-500  flex font-medium text-[.8rem]'>
                    {body.length <= 70 ? body : body.slice(0, 65)}
                    {body.length < 70 ? (
                        ''
                    ) : (
                        <p className='pt-[2px] cursor-pointer hover:text-blue-500 text-xs'>
                            ...See more
                        </p>
                    )}
                </p>
                <div className='text-[.60rem] flex gap-1 mt-1 text-slate-500'>
                    {tags.map((x) => {
                        return (
                            <span className='rounded-md pb-[1px] border dark:bg-slate-800/60 bg-slate-200/60 dark:border-slate-700 border-slate-400 w-fit px-2'>
                                {x}
                            </span>
                        );
                    })}
